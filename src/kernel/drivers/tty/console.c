...
send_updatemsg(void) {
	...
	if (get_jiffy() % (HZ / 10) == 0) {
		...
		send(TTY, &m,1);
		//printk("m.src : %d,m.type : %d\n",m.src,m.type);
		//printk("in TTY.messages:\n");
		//PCB *tmp = fetch_pcb(TTY);
		//ListHead *p;
		//list_foreach(p,&tmp->messages) 
		//	 printk("src : %d ,type : %x\n",((Msg *)(list_entry(p,Msg,list)))->src,
        //                            ((Msg *)(list_entry(p,Msg,list)))->type);
	}
}
